...
func Test_Backers_Are_In_Tournament(t *testing.T) {
	...
	actualBackers, err := l.checkBackersAreInTournament(tId, backerIds)
	if err != nil {
		t.Fatal(err)
	}

	assert.Equal(t, len(*backerIds), len(*actualBackers))
}

func Test_Backers_Are_Not_In_Tournament(t *testing.T) {
	...
	_, err = l.checkBackersAreInTournament(tId, backerIds)

	assert.Equal(t, err.Error(), "backer is not participating in tournament")
}
